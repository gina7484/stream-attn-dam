...
mod tests {
    ...
    fn qkt_red_div_matvec_test() {
        ...
        // Generators
        // Q = FIFO[T](N)
        let q_chan_depth: usize = if seq_len < 32000 {(seq_len*seq_len) as usize} else {256};
        let (q_sender, q_receiver) = ctx.bounded::<f64>(q_chan_depth);
        ...
        let kt_chan_depth: usize = if seq_len < 32000 {(seq_len*seq_len) as usize} else {256};
        let (kt_sender, kt_receiver) = ctx.bounded::<f64>(kt_chan_depth);
        let kt_iter =
        ...
        let v_chan_depth: usize = if seq_len < 32000 {(seq_len*seq_len) as usize} else {256};
        let (v_sender, v_receiver) = ctx.bounded::<f64>(v_chan_depth);
        let v_iter =
            || (0..(seq_len * seq_len)).map(|i| if i % seq_len == 0 { 0.11_f64 } else { 0.1_f64 });
